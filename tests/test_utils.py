...
import configparser
import pytest
from icon_generator.utils import *
...
def test_get_file_names(tmp_path: Path):
    ...
    assert set(get_file_names(str(tmp_path), '.png')) == set(['test1.png', 'test2.png'])
    assert set(get_file_names(str(tmp_path), '.txt')) == set(['test3.txt'])


def test_get_file_info():
    assert get_file_info('/usr/home/user/icon_generator/test.png') == ('test', '.png')
...
def test_load_image_files(tmp_path: Path):
    ...
    assert set(load_image_files(str(tmp_path), '.png')) == set([str(tmp_path / 'test1.png'), str(tmp_path / 'test2.png')])
    assert load_image_files(str(tmp_path), '.txt') == [str(tmp_path / 'test3.txt')]


def test_save_image(tmp_path: Path):
    new_image = Image.new('RGB', (60, 30), color='red')
    save_image(new_image, str(tmp_path), 'test.png')
