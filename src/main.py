...
from icon_generator.utils import (get_subfolder, get_filename)
load_dotenv()
...
for base_image_path in base_images:
    ...
    for mask_image_path in mask_images:
        ...
        combined_image = apply_mask(
             base_image_path, mask_image_path, proportion=0.4)
        save_image_variants(
                     combined_image, f"{output_folder_path}/{os_folder}/",category_name, filename, sizes)
