...
export const CanvasPreview = memo(forwardRef<HTMLDivElement, CanvasPreviewProps>(({ 
  folderImage, 
  iconName, 
  iconType, 
  iconColor, 
  ...
  const propsRef = useRef({ 
      osName, folderName, format, 
      displaySrc, // Use the proper display source (Blob or URL)
      // Also store styling props since they affect the capture
      iconName, iconType, iconColor, iconSize, offsetX, offsetY, 
      iconEffect, iconTransparency, folderHue 
  });
  ...
  useEffect(() => {
    propsRef.current = { 
      ...
      osName, folderName, format, displaySrc,
      iconName, iconType, iconColor, iconSize, offsetX, offsetY, 
      iconEffect, iconTransparency, folderHue
  ]);
  ...
  return (
    <div ref={containerRef} className="relative w-[512px] h-[512px] flex items-center justify-center bg-transparent">
      {/* Folder Base */}
      {displaySrc && (
        <img 
          key={displaySrc}
