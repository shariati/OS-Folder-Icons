...
function SignupContent() {
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      // After successful signup/login, move to subscription step if not already selected
      setStep(2);
    } catch (err) {
  ...
  const handlePlanSelection = (planId: string) => {
    ...
    // Here you would typically process the subscription or redirect to checkout
    // For this demo, we'll just redirect to home with a success message (simulated)
      ...
      <div className="w-full max-w-5xl">
        ...
        <div className="flex items-center justify-center mb-12">
          <div className="flex items-center">
            <div className={`flex items-center justify-center w-10 h-10 rounded-full font-bold transition-colors ${step >= 1 ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-500'}`}>
              1
            </div>
            <span className={`ml-3 font-medium ${step >= 1 ? 'text-blue-600 dark:text-blue-400' : 'text-gray-500'}`}>Account</span>
          </div>
          <div className={`w-24 h-1 mx-4 rounded-full ${step >= 2 ? 'bg-blue-600' : 'bg-gray-200 dark:bg-gray-700'}`}></div>
          <div className="flex items-center">
            <div className={`flex items-center justify-center w-10 h-10 rounded-full font-bold transition-colors ${step >= 2 ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-500'}`}>
              2
            </div>
            <span className={`ml-3 font-medium ${step >= 2 ? 'text-blue-600 dark:text-blue-400' : 'text-gray-500'}`}>Subscription</span>
          </div>
        </div>
        ...
        {step === 2 && (
          <div className="animate-fade-in-up">
            <div className="text-center mb-12">
              ...
              <p className="text-gray-500 dark:text-gray-400 text-lg">Select the plan that best fits your needs.</p>
            </div>
